...
public:
  ...
  void htCellPos   ( const int& id, const int& pos ) { htCellIds_   .push_back( id ); htCellPos_   [ id ] = pos; }
...
public:
  ...
  double digi( const double& val, const double& base ) const { return settings_->enableDigitize() ? ( floor( val / base ) + .5 ) * base : val; }
