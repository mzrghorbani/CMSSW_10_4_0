...
Track::Track( const Settings* settings, const int& htCell, const Stubs& stubs, const int& pos ) : settings_( settings ), parent_( nullptr ), stubs_( stubs ) {
  ...
  phi_     = reco::deltaPhi( ( binPhi_ + .5 ) * baseC + ( region_ + 1 ) * baseRegion + ( secPhi_ - .5 ) * baseSector - chosenRofPhi * invPtToDphi * qOverPt_, 0. );
...
Track::Track( const Track* mhtTrack, const Stubs& stubs, const double& qOverPt, const double& phi, const double& cot, const double& z ) :
  settings_( mhtTrack->settings_ ), parent_( mhtTrack ), stubs_( stubs )
{
  ...
  cellId_ = ( region_ * numSectorsPhi + secPhi_ * numSectorsEta + secEta_ ) * htNumCells * mhtNumCells + binPt_ * htNumBinsPhi * mhtNumBinsPhi + binPhi_;
